...
import { requireAdminOrInstructor, requireCreatorOrAdmin } from '../middleware/roles.js';
...
export async function getAllCourses(request, env, supabase) {
    ...
    const isPaid = url.searchParams.get('is_paid'); // 'true', 'false', or null for all
 ...
 * Supports multiple files: files[] field
 */
...
export async function updateCourse(request, env, supabase, user, courseId) {
    ...
    // Recalculate is_paid if price is updated
    if (updates.price !== undefined) {
        updates.is_paid = parseFloat(updates.price) > 0;
    }
 ...
 * Delete course (admin or course creator)
 */
 ...
 * Purchase a course (any authenticated user)
 */
export async function purchaseCourse(request, env, supabase, user) {
    ...
    // For paid courses, this would typically be called after payment verification
    const { data, error } = await supabase
        .from('user_courses')
        .insert([{
            user_id: user.userId,
