...
import { requireAdminOrAuthor, requireCreatorOrAdmin } from '../middleware/roles.js';
...
export async function createBlog(request, env, supabase, user) {
    // Admin or author check
    const roleError = requireAdminOrAuthor(user);
    if (roleError) return roleError;

    const { title, content, is_published = false } = await request.json();
